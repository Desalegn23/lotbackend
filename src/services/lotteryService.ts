...
export class AgentService {
  ...
  static async getAgentLotteryWinners(userId: string, lotteryId: string) {
    const agent = await prisma.agent.findUnique({
      where: { userId: userId },
    });
    ...
    if (!lottery) throw new Error('Lottery not found or access denied');
  ...
  static async getAgentLotteryTickets(userId: string, lotteryId: string) {
    const agent = await prisma.agent.findUnique({
      where: { userId: userId },
    });
